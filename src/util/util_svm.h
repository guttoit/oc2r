...
char* create_svm_header(int n_samples, int size_sample, int size_output);

int print_svm_input(float *ann_in, int n_inputs);
...
void predict(FILE *input, char *path_model, FILE *output)
{
	model = svm_load_model(path_model);
	int correct = 0;
	int total = 0;
	double error = 0;
